...
private:
    ...
    uint fontSize;
public:
    Text(GUI* gui, uintVec2 size, FontMetrics* metrics, std::string text, Alignment alignment, Color color, uint fontSize);
    ...
    void render(uintVec2 pos, std::list<float>& vertices, std::list<float>& texCoords, std::list<Color> &colors, std::list<uint> &texture);
};
