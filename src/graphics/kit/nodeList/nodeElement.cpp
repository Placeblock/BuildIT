...
NodeElement::NodeElement(GUI::View *view, const std::string& name, NodeReceiver* nodeReceiver, Sim::Simulation* simulation)
    : GUI::VerticalList(view, uintVec2(160, 176)), nodeReceiver(nodeReceiver), simulation(simulation) {
...
void NodeElement::onMouseAction(glm::vec2 relPos, int button, int mouseAction) {
    ...
    this->nodeReceiver->receiveNode(glm::vec2(this->getRelPos()) + relPos, std::move(this->createNode(glm::vec2(this->getAbsPos()) + relPos)));
}
