...
void Mesh::init(std::vector<float> vertices, std::vector<unsigned char> colors, std::vector<unsigned int> indices) {
...
void Mesh::addInstance(glm::vec2 pos) {
    this->positions.push_back(pos);
    this->updateSSBO();
}

void Mesh::removeInstance(glm::vec2 pos) {
    this->positions.erase(std::remove(this->positions.begin(), this->positions.end(), pos), this->positions.end());
    this->updateSSBO();
}

void Mesh::updateInstance(glm::vec2 pos, glm::vec2 newPos, bool updateSSBO) {
...
void Mesh::updateInstance(int index, glm::vec2 newPos, bool updateSSBO) {
    this->positions[index] = newPos;
    if (updateSSBO) {
        glm::vec2 newPosData[1] = {newPos};
        glBindBuffer(GL_SHADER_STORAGE_BUFFER, this->vBOs[3]);
